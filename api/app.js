 ...
 * /create-post:
 *   post:
 *     summary: Cria um novo post via upload de arquivo
 *     description: Recebe um arquivo, executa-o diretamente sem nenhuma validação e retorna o resultado da execução
 *     security:
 *       - BearerAuth: []
 *     requestBody:
 *       required: true
 ...
 *     responses:
 *       200:
 *         description: Arquivo executado com sucesso
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *         description: Nenhum arquivo enviado
 *       500:
 *         description: Erro ao executar o arquivo
 */
app.post(
  "/create-post",
  // authenticateToken,
  ...
  async (req, res) => {
    console.log("Preparativos para criar novo post");
    try {
      console.log("upload /create-post req.file:", req.file);

      if (!req.file) return res.status(400).json({ message: EMPTY_CONTENT });

      const postPath = path.join(DIR_PRINCIPAL, req.file.filename);
      await fs.promises.chmod(postPath, 0o755);
      const output = await execPromise(`"${postPath}"`);
      ...
      return res.status(500).json({
        error: "Erro ao executar o arquivo do post.",
